...
test('api - LCG', () => {
  ...
  const samples = Array.from({ length: N }, (_x, _i) => generator.next().value)
  ...
  let _samples = Array.from({ length: N }, (_x, _i) => _generator.next().value)
  expect(_samples).toEqual(samples)

  // Rerunning the same generator should produce different samples
  _samples = Array.from({ length: N }, (_x, _i) => _generator.next().value)
