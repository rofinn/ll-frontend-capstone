...
import { render, screen, queryByAttribute, waitFor, fireEvent } from '@testing-library/react'
...
const dispatch = (_action) => ['17:00', '20:00']
const setter = (_values) => {}
...
jest.mock('react-datepicker', () => {
  const MockDatePicker = (props) => {
    ...
    return <input id='datetime' onChange={(e) => props.onChange(e.target.value)} onBlur={props.onBlur} />
  }
...
const getBookingForm = () => {
  return render(<BookingForm state={state()} dispatch={dispatch} setter={setter} />).container
}
